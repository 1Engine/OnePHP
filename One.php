...
class One {
		...
		$this->settings['method'] = [
			...
			'method' => function($params) {
				if (is_callable($this->settings['method'][$name])) {
					$this->settings['method'][$name]($params);
				}
			},
			'run' => function($params) {
			...
			'gzipText' => function($text, $level = 3, $file = null) {
			...
			'gunzipText' => function($text) {
				return gzdecode($text);
			},
			'gunzipTextFromFile' => function($file) {
			    $string = "";
			    if (file_exists($file)) {
				    $fp = gzopen($file, "rb");
			    	while (!gzeof($fp)) {
	...
	private function settings($params, $settings = []) {
		if (is_string($params)) {
